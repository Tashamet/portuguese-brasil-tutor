...
#
#   remote-setup.sh <send_time HH:MM> <data_repo_url>
set -euo pipefail
...
echo "[remote-setup] code=${CODE} data=${DATA} send_time=${SEND_TIME}"
...
( crontab -l 2>/dev/null | grep -v 'cli/tutor.py notify' ; echo "${CRON_LINE}" ) | crontab -
